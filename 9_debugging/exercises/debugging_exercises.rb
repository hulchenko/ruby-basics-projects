...
def increment_greatest_value(nested_array) # [[3, 4], [1, 2]]
  greatest_value = nested_array.flatten.min # 1
  nested_array.each do |array| # [3, 4] and then [1, 2]
    array.each do |current_value|
...
def isogram?(string)
  p "STRING: #{string}" #Odin
  original_length = string.length #4
  string_array = string.downcase.split('') # <- should be .split('')?
  p "ARRAY: #{string_array}"
