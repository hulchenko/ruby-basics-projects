def find_language_information(languages, language_name)
  ...
  # languages.select { |language, data| data if language == language_name  } <-- doesn't work
...
def add_information_about_language(languages, language_name, info_key, info_value)
  ...
  languages.select do |language, data|
    if language == language_name
      languages[language.to_sym][info_key.to_sym] = info_value
    end
    languages
  end
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  languages.dig(language_name, :facts, fact_index) # [] -> :sym -> :sym -> ''
end
